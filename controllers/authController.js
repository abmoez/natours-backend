...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser)
    return next(
      new AppError(
        'The user of this token is no longer exist',
        401
      )
    );

  // 4) Check if user changer password
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError(
          'You dont have peprmission to perform this action',
          403
        )
      );
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // happend behind scenes in pre-save middleware
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  console.log(user);
  // 2) Check if POSTed passwordCurrent is correct
  if (
